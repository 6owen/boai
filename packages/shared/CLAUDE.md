...
## Notes
- `PiAgent` is the only Agent Backend. Model suppliers are routed by `piAuthProvider`.
...
- On Windows, the validated `gitBashPath` is forwarded to the PI subprocess as `CRAFT_PI_SHELL_PATH`; do not restore Claude Code environment variables.
- MCP proxy tool names are built ONLY via `proxyToolName(slug, name)` (`mcp/proxy-tool-name.ts`), which sanitizes characters outside `[a-zA-Z0-9_-]`. Build, emit, PI registration, and dispatch must use this one builder or the exact-match key drifts. Post-sanitization collisions keep the first tool and warn for the skipped one.
...
- `LlmConnection.midStreamBehavior` controls whether mid-stream sends steer the active PI turn or queue for the next turn. Read it through `resolveMidStreamBehavior(connection)`. A queued message is re-stamped by `processNextQueuedMessage` so it sorts after the finalized assistant reply.
- The network interceptor (`unified-network-interceptor.ts`) is PI-only. Development loads the TypeScript source; packaged builds load `apps/electron/dist/interceptor.cjs` through Electron's embedded Node runtime.
- Per-message context is split into **volatile** vs **stable** blocks. PI folds stable blocks into the system prefix and routes volatile blocks to the user tail. `buildVolatileContextParts` consumes a one-shot signal, so call it exactly once per turn.
...
- **Main** hydrates on startup from `preferences.uiLanguage` in `~/.boai/preferences.json`. Maintained only by the `i18n:changeLanguage` IPC handler in `apps/electron/src/main/index.ts`.
