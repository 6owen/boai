...
const BOAI_CONFIG_DIR = process.env.CRAFT_CONFIG_DIR || join(homedir(), '.boai');
export const CONFIG_FILE = join(BOAI_CONFIG_DIR, 'config.json');
...
export const LOG_DIR = join(BOAI_CONFIG_DIR, 'logs');
...
function getErrorFilePath(): string {
  ...
  return join(BOAI_CONFIG_DIR, 'api-error.json');
}
