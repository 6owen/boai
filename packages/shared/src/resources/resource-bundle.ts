...
export function exportResources(
  ...
  // Old callers may still send this option. Keep the request non-fatal while
  // making sure BoAI never serializes executable automation definitions.
  if (options.automations) {
    warnings.push('Automations are no longer exported by BoAI and were ignored')
  }
...
export async function importResources(
  ...
  const warnings: string[] = []
  if (Array.isArray(bundle.resources.automations) && bundle.resources.automations.length > 0) {
    const count = bundle.resources.automations.length
    warnings.push(`Ignored ${count} legacy automation${count === 1 ? '' : 's'}; BoAI no longer imports automations`)
  }
