...
export function getPathHint(targetPath: string, plansFolderPath: string, dataFolderPath?: string): string | null {
  ...
  if ((normalizedTarget.includes('/.boai/workspaces/') || normalizedTarget.includes('/.craft-agent/workspaces/')) && !normalizedTarget.includes('/sessions/')) {
    return 'Hint: Write to the session plans or data folder, not the workspace root.';
  }

  // Case: Writing outside the current or legacy BoAI data root entirely
  if (!normalizedTarget.includes('/.boai/') && !normalizedTarget.includes('/.craft-agent/')) {
