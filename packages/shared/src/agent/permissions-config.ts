 ...
 *
 * File locations:
 * - Workspace: ~/.boai/workspaces/{slug}/permissions.json
 * - Per-source: ~/.boai/workspaces/{slug}/sources/{sourceSlug}/permissions.json
 *
 ...
 * Default permissions are stored at ~/.boai/permissions/
 * Reads env vars dynamically so tests can override via BOAI_HOME or the
 * deprecated CRAFT_CONFIG_DIR alias.
 */
export function getAppPermissionsDir(): string {
  const configDir = resolveConfigDir().path;
...
/**
 * Load default permissions from ~/.boai/permissions/default.json
...
class PermissionsConfigCache {
  ...
  // App-level default permissions (loaded from ~/.boai/permissions/default.json)
   ...
   * These come from ~/.boai/permissions/default.json
   */
