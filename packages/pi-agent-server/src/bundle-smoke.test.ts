...
function driveBundle(messages: object[], done: (output: string) => boolean): Promise<string> {
  return new Promise((resolve, reject) => {
    // The packaged subprocess runs under Electron's embedded Node runtime, not Bun.
    // Running the smoke bundle with process.execPath from bun:test would exercise
    // Bun's partial node:worker_threads compatibility instead of the production runtime.
    const child = spawn(resolveSystemNode(), [bundlePath], { cwd: scratchDir, stdio: ['pipe', 'pipe', 'pipe'] });
    let output = '';
