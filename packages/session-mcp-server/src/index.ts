 ...
 *   --workspace-root: Path to workspace folder (~/.boai/workspaces/{id})
...
function createCodexContext(config: SessionConfig): SessionToolContext {
  ...
  return {
    ...
    updatePreferences: (updates: Record<string, unknown>) => {
      ...
      // workspaceRootPath = ~/.boai/workspaces/{id}
      // preferencesPath = ~/.boai/preferences.json
    ...
    submitFeedback: (feedback) => {
      const configDir = process.env.BOAI_HOME
