...
app.whenReady().then(async () => {
  ...
  // Seed tool icons to ~/.boai/tool-icons/ (copies bundled SVGs on first run)
  ensureToolIcons()

  // Seed preset themes to ~/.boai/themes/ (copies bundled theme JSONs on first run)
  ensurePresetThemes()
        ...
        createHandlerDeps: ({ sessionManager: sm, platform: p, oauthFlowStore: ofs }) => {
          ...
          messagingHandle = createMessagingBootstrap({
            ...
            getMessagingDir: (wsId: string) =>
              join(APP_PATHS.workspacesDir, wsId, 'messaging'),
            ...
            // at ~/.boai/logs/messaging-gateway.log.
