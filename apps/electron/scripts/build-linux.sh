...
Environment variables (from .env or environment):
  ...
  BOAI_SKIP_INSTALL=1       - Reuse dependencies already installed at repository root
EOF
    exit 0
}
...
cd "$ROOT_DIR"
if [ "${BOAI_SKIP_INSTALL:-0}" = "1" ]; then
    echo "Reusing existing dependencies (BOAI_SKIP_INSTALL=1)..."
else
...
# 5. The interceptor is bundled into dist/interceptor.cjs for the PI Node
#    subprocess. Raw TypeScript sources are not staged into the application.
...
cd "$ROOT_DIR"
export BOAI_BUILD_PLATFORM=linux
export BOAI_BUILD_ARCH="$ARCH"
...
UNPACKED_PATH="$ELECTRON_DIR/release/linux-unpacked"
bun "$ROOT_DIR/scripts/validate-electron-artifact.ts" "$UNPACKED_PATH"
bun "$ROOT_DIR/scripts/electron-artifact-report.ts" "$UNPACKED_PATH" \
  --json "$ELECTRON_DIR/release/artifact-report-linux-${ARCH}.json"
