...
cd "$ROOT_DIR"
export BOAI_BUILD_PLATFORM=darwin
...
APP_PATH="$ELECTRON_DIR/release/$MAC_DIR/BoAI.app"
bun "$ROOT_DIR/scripts/validate-electron-artifact.ts" "$APP_PATH"
bun "$ROOT_DIR/scripts/electron-artifact-report.ts" "$APP_PATH" \
  --json "$ELECTRON_DIR/release/artifact-report-mac-${ARCH}.json"
