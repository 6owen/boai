...
This guide explains how to create and configure skills in BoAI.
...
BoAI uses **the identical SKILL.md format** as the Claude Code SDK. This means:

1. **Format compatibility**: Any skill written for Claude Code works in BoAI
...
1. **Workspace skill checked first** - If `~/.boai/workspaces/{id}/skills/commit/SKILL.md` exists, it's used
...
```
~/.boai/workspaces/{workspaceId}/skills/{slug}/
...
```bash
mkdir -p ~/.boai/workspaces/{ws}/skills/my-skill
```
...
1. Create `~/.boai/workspaces/{ws}/skills/commit/SKILL.md`
