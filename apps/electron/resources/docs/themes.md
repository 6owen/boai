...
This guide explains how to customize the visual theme of BoAI.

## Overview

BoAI uses a 6-color theme system with support for both app-level defaults and per-workspace overrides.
...
3. **Preset themes**: `~/.boai/themes/{name}.json` - Complete theme packages
4. **Theme overrides**: `~/.boai/theme.json` - Override specific colors (app-level)
...
```
~/.boai/workspaces/{id}/config.json
```
...
Create `~/.boai/theme.json` to override specific colors:
...
Preset themes are complete theme packages stored at `~/.boai/themes/`. Each preset is a JSON file with theme colors and metadata.
...
2. Save it to `~/.boai/themes/{name}.json`
...
1. Create `~/.boai/theme.json` for overrides or `~/.boai/themes/{name}.json` for a preset
...
- Check file is in correct location (`~/.boai/theme.json` for overrides, `~/.boai/themes/` for presets)
